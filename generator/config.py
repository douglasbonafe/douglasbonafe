...
def validate_config(config: dict) -> dict:
    ...
    # profile — supports both 'name' (original) and 'nome' (Portuguese alias)
    profile = config.get("profile", {})
    if not isinstance(profile, dict):
        raise ConfigError("'profile' must be a mapping.")
    # Normalize: 'nome' → 'name' for compatibility with galaxy-profile templates
    if profile.get("nome") and not profile.get("name"):
        profile["name"] = profile["nome"]
    # Normalize: 'titulo' → 'tagline', 'localizacao' → 'location', 'empresa' → 'company'
    if profile.get("titulo") and not profile.get("tagline"):
        profile["tagline"] = profile["titulo"]
    if profile.get("localizacao") and not profile.get("location"):
        profile["location"] = profile["localizacao"]
    if profile.get("empresa") and not profile.get("company"):
        profile["company"] = profile["empresa"]
    if not profile.get("name"):
        raise ConfigError("'profile.name' or 'profile.nome' is required.")
    ...
    # theme — optional; None when all entries are commented out in YAML
    user_theme = config.get("theme") or {}
